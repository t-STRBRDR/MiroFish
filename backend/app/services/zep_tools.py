    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                if skip:
                    continue
                # Truncate long content (by period, not hard truncation)
...
class ZepToolsService:
    ...
    def search_graph(
        ...
        Falls back to local keyword matching if the search API is unavailable.
        ...
        try:
            ...
            result = adapter.search(query, limit=limit, scope=scope)
            ...
            return SearchResult(
                ...
                nodes=nodes_data,
                query=query,
                total_count=len(facts) + len(nodes_data)
            )
        except Exception as e:
            logger.warning(f"Search failed, falling back to local search: {e}")
    ...
    def get_node_detail(self, graph_id: str, node_uuid: str) -> Optional[NodeInfo]:
        ...
        try:
            adapter = GraphitiAdapter.get_or_create(graph_id)
            node = adapter.get_node(node_uuid)
            if node:
                return NodeInfo(
                    uuid=node.get("uuid", ""),
                    name=node.get("name", ""),
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        Gets all edges in the graph, then filters those related to the specified node.
        ...
        try:
            ...
            result = [EdgeInfo(
                ...
                invalid_at=e.get("invalid_at"),
                expired_at=e.get("expired_at"),
            ) for e in all_edges if e.get("source_node_uuid") == node_uuid or e.get("target_node_uuid") == node_uuid]
...
Return the sub-questions as a JSON list."""
    ...
    def interview_agents(
        ...
        4. Call /api/simulation/interview/batch endpoint for real interviews (dual platform)
