...
class OasisProfileGenerator:
    ...
    def __init__(
        ...
        # Graphiti adapter for knowledge graph search
        self.graph_id = graph_id
        self._graphiti_adapter = None
        if self.graph_id:
            try:
                self._graphiti_adapter = GraphitiAdapter.get_or_create(self.graph_id)
            except Exception as e:
                logger.warning(f"GraphitiAdapter initialization failed: {e}")
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use Graphiti graph search to obtain rich information related to entity
        ...
        try:
            # Search edges (facts/relations)
            edge_results = self._graphiti_adapter.search(
            ...
            node_results = self._graphiti_adapter.search(
                query=comprehensive_query,
                limit=20,
                scope="nodes"
            )
