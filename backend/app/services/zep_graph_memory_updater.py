...
class ZepGraphMemoryUpdater:
    ...
    Monitors simulation action logs and sends agent activities to the Zep graph in real-time.
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        Args:
            graph_id: Graph ID
            api_key: Reserved parameter, no longer used
        """
    ...
    def _send_batch_activities(self, activities: List[AgentActivity], platform: str):
        """
        Send batched activities to the Zep graph (merged as one text)
        """
        if not activities:
            return
    ...
    def _flush_remaining(self):
        ...
        # Send remaining activities from each platform buffer
        with self._buffer_lock:
