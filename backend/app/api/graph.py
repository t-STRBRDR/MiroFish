...
Using project context mechanism with server-side persistent state
"""
...
def reset_project(project_id: str):
    """
    Reset project state (for rebuilding knowledge graph)
    """
    project = ProjectManager.get_project(project_id)
...
def build_graph():
    """
    Endpoint 2: Build knowledge graph by project_id
    ...
    try:
        ...
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "Knowledge graph is being built, please do not resubmit. To force rebuild, add force: true",
        ...
        task_id = task_manager.create_task(f"Build knowledge graph: {graph_name}")
        ...
        def build_task():
            ...
            try:
                ...
                # Graphiti processes episodes synchronously, no waiting needed
        ...
        return jsonify({
            ...
            "data": {
                "project_id": project_id,
                "task_id": task_id,
                "message": "Knowledge graph build task started，query progress via /task/{task_id} "
            }
        })
